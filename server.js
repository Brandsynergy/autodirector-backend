...
// static: run artifacts (screenshots/videos/pdfs)
app.use("/runs", express.static(path.join(__dirname, "runs")));

const runs = new Map(); // id -> {status, logs[], shots[], pdfUrl?, videoUrl?}
...
function planFromPrompt(prompt) {
  ...
  const emailToMatch = p.match(/\bemail\s+to\s+([^\s"“”']+)|\bto:\s*([^\s"“”']+)/i);
  let to = (emailToMatch && (emailToMatch[1] || emailToMatch[2])) || null;
  if (to && (to.includes("…") || to.includes("..."))) to = null;
